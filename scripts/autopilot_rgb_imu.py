...
import json
import torch
from hardware_rgb_imu import get_realsense_frame, read_imu_data, setup_realsense_camera, setup_serial, setup_joystick, encode_dutycylce
...
# SETUP
# Load model
onnx_model_path = os.path.join('models', 'efficientnet_b2_rgb_imu.onnx')  # Update ONNX file path
model = torch.hub.load('pytorch/vision:v0.10.0', 'efficientnet_b2', pretrained=False)
model.classifier = torch.nn.Linear(model.classifier[1].in_features, 5)  # RGB + IMU requires 5 outputs
model.eval()
...
try:
    while True:
        ...
        # Process the frame and IMU data for prediction
        ...
        combined_input = torch.cat((img_tensor.view(1, -1), imu_tensor), dim=1)  # Concatenate RGB and IMU tensors
