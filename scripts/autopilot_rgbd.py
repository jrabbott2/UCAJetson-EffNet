...
import json
import torch
from hardware_rgbd import get_realsense_frames, setup_realsense_camera, setup_serial, setup_joystick, encode_dutycylce
...
# SETUP
# Load model
onnx_model_path = os.path.join('models', 'efficientnet_b2_rgbd.onnx')  # Update ONNX file path
model = torch.hub.load('pytorch/vision:v0.10.0', 'efficientnet_b2', pretrained=False)
model.classifier = torch.nn.Linear(model.classifier[1].in_features, 2)  # RGBD requires 2 outputs
model.eval()
...
try:
    while True:
        ...
        # Process the frames for prediction
        rgb_tensor = to_tensor(cv.resize(color_frame, (240, 240))).unsqueeze(0)  # Resize to 240x240 and add batch dimension
        depth_tensor = to_tensor(cv.resize(depth_frame, (240, 240))).unsqueeze(0)  # Resize depth to 240x240
        combined_input = torch.cat((rgb_tensor, depth_tensor), dim=1)  # Concatenate RGB and Depth tensors
