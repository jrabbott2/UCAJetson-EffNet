...
def setup_realsense_camera():
    """
    Configure RealSense camera pipeline for RGB and Depth streams at 424x240 resolution.
    """
    pipeline = rs.pipeline()
    config = rs.config()
    config.enable_stream(rs.stream.color, 424, 240, rs.format.bgr8, 60)  # RGB stream at 60 FPS
    config.enable_stream(rs.stream.depth, 424, 240, rs.format.z16, 60)  # Depth stream at 60 FPS
...
def get_realsense_frames(pipeline):
    ...
    depth_image_resized = cv.resize(depth_image, (240, 240))

    # Normalize depth image for visualization
    depth_image_normalized = cv.normalize(depth_image_resized, None, 0, 255, cv.NORM_MINMAX).astype(np.uint8)

    return True, color_image_resized, depth_image_normalized
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            success, color_frame, depth_frame = get_realsense_frames(pipeline)
            if not success:
                continue

            # Stack and display color and depth frames
            stacked_frames = np.hstack((color_frame, cv.cvtColor(depth_frame, cv.COLOR_GRAY2BGR)))
            cv.imshow('RGB and Depth Stream', stacked_frames)
