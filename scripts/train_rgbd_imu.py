...
class BearCartDataset(Dataset):
    """
    Customized dataset for RGB-D and IMU data.
    """
    ...
    def __getitem__(self, idx):
        ...
        # IMU data (columns 2-7 in labels.csv)
        imu_data = self.img_labels.iloc[idx, 2:8].to_numpy(dtype=np.float32)
        imu_tensor = torch.tensor(imu_data, dtype=torch.float32)

        # Steering and throttle values
        steering = self.img_labels.iloc[idx, 8].astype(np.float32)
        throttle = self.img_labels.iloc[idx, 9].astype(np.float32)

        return image_tensor.float(), imu_tensor, steering, throttle

def train(dataloader, model, loss_fn, optimizer):
    model.train()
    num_used_samples = 0
    ep_loss = 0.
    for b, (rgbd, imu, st, th) in enumerate(dataloader):
        target = torch.stack((st, th), dim=-1)
        rgbd, imu, target = rgbd.to(DEVICE), imu.to(DEVICE), target.to(DEVICE)
        pred = model(rgbd, imu)
...
class EfficientNetB2RGBDIMU(nn.Module):
    """
    EfficientNet-B2 modified to handle RGB-D and IMU data.
    """
    def __init__(self):
        super(EfficientNetB2RGBDIMU, self).__init__()
        self.rgbd_model = efficientnet_b2(weights=None)
        self.rgbd_model.features[0][0] = nn.Conv2d(4, 32, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1), bias=False)  # Adjust input channels to 4
        self.rgbd_model.classifier = nn.Identity()  # Remove default classifier
        ...
        self.fc = nn.Sequential(
            nn.Linear(self.rgbd_model.classifier[1].in_features + 32, 128),
            nn.ReLU(),
            nn.Linear(128, 2)  # Output steering and throttle
        )

    def forward(self, rgbd, imu):
        rgbd_features = self.rgbd_model(rgbd)
        imu_features = self.imu_fc(imu)
        combined = torch.cat((rgbd_features, imu_features), dim=1)
...
# Create model
model = EfficientNetB2RGBDIMU().to(DEVICE)
...
for t in range(epochs):
    ...
    if ep_test_loss < best_loss:
        ...
        model_name = f'efficientnet_b2_rgbd_imu-{t+1}ep-{lr}lr-{ep_test_loss:.4f}mse'
...
plt.grid(True)
plt.legend()
plt.title('EfficientNet-B2 Training with RGB-D and IMU')
plt.savefig(os.path.join(data_dir, 'efficientnet_b2_rgbd_imu_training.png'))

# Save the model (weights only)
torch.save(model.state_dict(), os.path.join(data_dir, 'efficientnet_b2_rgbd_imu_final.pth'))
print("Model weights saved")

# ONNX export
dummy_input_rgbd = torch.randn(1, 4, 240, 240).to(DEVICE)  # Adjust shape for 240x240 RGB-D
dummy_input_imu = torch.randn(1, 6).to(DEVICE)  # IMU data with 6 features
onnx_model_path = os.path.join(data_dir, 'efficientnet_b2_rgbd_imu.onnx')
torch.onnx.export(model, (dummy_input_rgbd, dummy_input_imu), onnx_model_path, opset_version=11)
